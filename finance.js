...
function guessCat(desc, cats) {
  ...
  // Delivery
  if (/UBER EATS|DOORDASH|GRUBHUB|POSTMATES|FANTUAN|SEAMLESS|CAVIAR|GOPUFF|MERCATO/.test(u)) return 'Delivery / Takeout';
  // Transport
  if (/UBER[^ ]|LYFT|WAYMO|TAXI|CLIPPER|TRANSIT|CALTRAIN|MUNI|METRO|FERRY|AMTRAK|LYFT|VIA\s/.test(u)) return 'Transport';
  // Gas
  if (/SHELL|EXXON|CHEVRON|BP\s|SUNOCO|SPEEDWAY|7[- ]ELEVEN|CIRCLE K|MOBIL|TEXACO|ARCO|VALERO/.test(u)) return 'Gas / Fuel';
  // Education
  if (/ITALKI|DUOLINGO|COURSERA|UDEMY|SKILLSHARE|BRILLIANT|MASTERCLASS|CODEACADEMY|KHAN\s/.test(u)) return 'Education';
  // Travel
  if (/HOTEL|AIRBNB|VRBO|EXPEDIA|BOOKING|DELTA|UNITED AIRLINES|SOUTHWEST|AMERICAN AIR|JETBLUE|SPIRIT|ALASKA AIR|FRONTIE|KAYAK|PRICELINE|TRIPADVISOR/.test(u)) return 'Travel';
  // Groceries
  if (/TRADER JOE|SAFEWAY|WHOLE FOODS|KROGER|COSTCO|WALMART GROCERY|SPROUTS|ALDI|PUBLIX|H-E-B|PCC\s|WINCO|TRADER\s/.test(u)) return 'Groceries';
  // Food & Dining
  if (/RESTAURANT|CAFE|COFFEE|STARBUCKS|MCDONALD|CHIPOTLE|PIZZA|TACO|SUSHI|RAMEN|BAR |BURGER|WENDY|KFC|SUBWAY|DOMINO|PANERA|DUNKIN|CHICK.FIL|POPEYES|JERSEY MIKE|IN-N-OUT|TACO BELL|WINGSTOP|JIMMY JOHN|FIVE GUYS|P.F\. CHANG|OLIVE GARDEN|CHEESECAKE FACT/.test(u)) return 'Food & Dining';
  // Utilities
  if (/PG&E|COMED|CON ED|DUKE ENERGY|DOMINION|SOUTHERN CAL|SDGE|LADWP|AT&T|VERIZON|T-MOBILE|SPECTRUM|COMCAST|XFINITY|CHARTER|COX\s/.test(u)) return 'Utilities';
  // Insurance
  if (/GEICO|PROGRESSIVE|STATE FARM|ALLSTATE|LIBERTY MUTUAL|NATIONWIDE|TRAVELERS|METLIFE|AETNA|CIGNA|BLUE CROSS|KAISER/.test(u)) return 'Insurance';
  // Pharmacy / Health
  if (/CVS|WALGREEN|WALGREENS|RITE AID|WAL-MART PHARMACY|TARGET PHARMACY|KROGER PHARMACY|SAFEWAY PHARMACY/.test(u)) return 'Pharmacy';
  // Personal Care
  if (/BARBER|SALON|SPA|ULTA|SEPHORA/.test(u)) return 'Personal Care';
  // Shopping
  if (/AMAZON|BESTBUY|APPLE STORE|TARGET|NORDSTROM|ZARA|H\&M|WALMART[^G]|HOME DEPOT|LOWES|IKEA|COSTCO|KOHL|MACY|BED BATH|NORDSTROM|TJ MAXX|MARSHALLS|EBAY|ETSY|WISH|ALIEXPRESS/.test(u)) return 'Shopping';
  // Entertainment
  if (/NETFLIX|SPOTIFY|HULU|DISNEY|APPLE\.COM|APPLE\.TV|APPLE MUSIC|GOOGLE ONE|GOOGLE PLAY|YOUTUBE|HBO|MAX\s|PARAMOUNT|PEACOCK|PRIME VIDEO|AUDIBLE|PATREON|SUBSCRIPTION/.test(u)) return 'Entertainment';
  // Health & Fitness
  if (/GYM|FITNESS|YOGA|PILATES|PELOTON|PLANET FITNESS|BARRE|ORANGETHEORY|SOULCYCLE|CLASSPASS/.test(u)) return 'Health & Fitness';
  return 'Other';
}
